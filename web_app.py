...
class IndexHandler(tornado.web.RequestHandler):
    ...
    async def get(self):
        ...
        msg += f"<p><a href=\"https://github.com/zRitsu/DC-MusicBot-RPC" \
              f"/releases\" target=\"_blank\">Download rich presence app here.</a></p>Link to add to the app " \
              f"de RPC: {ws_url}"
                ...
                if self.auth_enabled:
                    ...
                    if users_ws[data["user"]].token != token:
                        ...
                        data.update(
                            {
                                "op": "exception",
                                "message": "Invalid token! Just in case, generate a new token using the command no "
                                           "bot: /rich_presence."
                            }
                        )
